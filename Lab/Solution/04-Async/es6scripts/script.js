...
function doIt() {
  ...
  const p1 = new Promise(function (resolve, reject) {
    ...
    displayMessage(
      "info",
      `Promise ${thisPromiseCount}, async code started, random number is ${rand}`
    );
    window.setTimeout(() => {
      if (rand < 0.5) resolve(`Promise ${thisPromiseCount}, resolved`);
      else reject(`Promise ${thisPromiseCount}, rejected`);
    }, rand * 5000 + 1000);
  });

  // Handle Promise completion - then() handles successful resolution, catch() handles rejection.
  p1.then((val) => displayMessage("resolved", val)).catch((reason) =>
