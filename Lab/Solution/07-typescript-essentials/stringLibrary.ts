function toTitleCase(input: string): string {
  ...
  for (const word of words) {
    ...
    // join the initial and the rest and add them to the resultingarray
...
function toWords(input: string): string[] {
  const words = input.split(/[ _-]/);
  // return the words that were split
  return words;
}
...
function isAlpha(input: string): boolean {
  // regex that will match any string that only has upper and
  //lowercase letters
